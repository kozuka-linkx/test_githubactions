...
class WhiteBalanceOffsetRed(BaseParameterIDS):
    """
    ホワイトバランス(赤)クラス．
    ...
    get_min_value() -> int
        抽象メソッドをオーバーライド．
        設定可能なホワイトバランス(赤)の最小値を返すメソッド．

    get_max_value() -> int
        抽象メソッドをオーバーライド．
        設定可能なホワイトバランス(赤)の最大値を返すメソッド．

    get_now_value() -> float
        抽象メソッドをオーバーライド．
        ホワイトバランス(赤)の現在値を返すメソッド．
    ...
    def __init__(self, camera):
        ...
        # またホワイトバランス(赤)変更，取得時にホワイトバランス(青)も
    ...
    def value(self):
        ...
        実行してホワイトバランス(赤)の最大値，最小値，現在値，
        ホワイトバランス自動調整がオンになっているかを取得する．

        Returns
        --------------------------------
        values: dict
            "max": ホワイトバランス(赤)の最大値(float)
            "min": ホワイトバランス(赤)の最小値(float)
            "now": ホワイトバランス(赤)の現在値(float)
    ...
    def value(self, value=None):
        ...
        外部からset_parameter(ホワイトバランス(赤)変数名)とset_parameter(
        ホワイトバランス(赤)変数名, value: float)が実行された場合で異なる挙動をする．

        set_parameter(ホワイトバランス(赤)変数名, value: float) -> None
            ホワイトバランス(赤)をvalueに変更する．
            ただしホワイトバランス自動調整がオンの場合には標準出力に警告文を
            出力するのみ．

        set_parameter(ホワイトバランス(赤)変数名) -> None
        ...
        if auto_gain is False:
            ...
            if not isinstance(value, bct.DummyValue):
                ...
                # 現在のホワイトバランス(赤)を取得するためにget_now_value実行．
                _ = self.get_now_value()
                # 値書き換え
                nRet = ueye.is_SetAutoParameter(
                    self.__camera, ueye.IS_SET_AUTO_WB_OFFSET,
                    new_value, self.__sub_value
                )
    ...
    def get_now_value(self):
        ...
        nRet = ueye.is_SetAutoParameter(
            ...
            self.__now_value, self.__sub_value
        )
